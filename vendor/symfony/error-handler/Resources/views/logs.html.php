...
foreach ($logs as $log) {
    if ($log['priority'] >= 400) {
        ...
        $severity = 0;
        if (($exception = $log['context']['exception'] ?? null) instanceof \ErrorException || $exception instanceof \EasyCI202307\Symfony\Component\ErrorHandler\Exception\SilencedErrorContext) {
